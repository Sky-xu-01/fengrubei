...
def interpolate_to_grid(points, values_dict, nx, ny, nz):
    """CPU interpolation: linear + nearest fill for NaN."""
    x = points[:, 0]
    y = points[:, 1]
    z = points[:, 2]
    ...
    for name, values in values_dict.items():
        print(f"    Interpolating {name}... [CPU linear]")
        grid_values = griddata(points, values, (grid_x, grid_y, grid_z), method='linear')

        mask = np.isnan(grid_values)
        if np.any(mask):
            grid_values_nearest = griddata(points, values, (grid_x, grid_y, grid_z), method='nearest')
            grid_values[mask] = grid_values_nearest[mask]

        interpolated_data[name] = grid_values
...
def main():
    ...
    print("Interpolation backend: CPU (SciPy linear + nearest fill)")
